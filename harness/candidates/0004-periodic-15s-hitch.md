...
- Status: PARKED / RESOLVED — a 2h16m long-session log settles it: the growth is **Boehm fragmentation + legitimate colony growth, NOT a hard object leak.** Reserved/Used ratio collapses 0.96 → 0.55 (reserved grows ~5x faster than the live-object floor); the used-floor slope DECELERATES per quartile (14 → 4.6 → 8.6 → 3.7 MB/min) rather than climbing steadily; process +5.5 GB but only +~1.1 GB managed live-set. My earlier "leak ~18 MB/min" (7-min idle) was too strong — the audit's Boehm-fragmentation caveat was right. GC pauses get FEWER but BIGGER over a session (mean frame +80%, worst sweeps 3.2-3.7s late) because Boehm scans the whole growing heap. Largely INHERENT to a big late-game colony on Boehm; not a fixable mod-side target. Full data: `harness/longsession-log-audit.md`.
